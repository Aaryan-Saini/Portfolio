/* ============================================================
   main.js → siteEffects.js (Next.js port)
   GSAP + ScrollTrigger + Lenis, imported from npm instead of CDN.
   Highlights:
...
export default function initSiteEffects() {
  ...
  /* ---------- Sound toggle wiring (shared by loader + header) ---------- */
  function wireSoundButton(btn, label) {
    if (!btn) return;
    const render = (on) => {
    ...
    audio.subscribe(render); // keeps every toggle in sync with the shared state
    btn.addEventListener("click", () => { audio.toggle(); });
    btn.addEventListener("mouseenter", () => audio.hover());
  }
  function wireAudioUI() {
    wireSoundButton($("[data-site-audio]"), $("[data-site-audio-label]"));
  }

  /* ---------- Hero — cinematic cursor/scroll-reactive smoke (WebGL) ---------- */
  function heroSmokeGL() {
    ...
    const FRAG = [
      ...
      "void main(){",
      "  vec2 uv=v_uv; float asp=u_res.x/max(u_res.y,1.0);",
      "  vec2 p=vec2((uv.x-0.5)*asp, uv.y-0.5)*2.4;",
      "  float t=u_time*0.05;",
      "  p.y -= t*0.30 + u_scroll*0.55;",                    // rises faster as you scroll
      "  vec2 m=vec2((u_mouse.x-0.5)*asp,(u_mouse.y-0.5));",
      "  p += m*0.30;",                                       // domain drifts toward the cursor
      ...
      "  float core=smoothstep(0.95,0.0,length(c));",
      "  float pulse=0.86+0.14*sin(u_time*0.6);",
      "  float coreI=core*pulse*(0.55+0.7*u_scroll);",        // the shared 'heart' blooms on scroll
      "  float mb=smoothstep(0.45,0.0,length(vec2((uv.x-u_mouse.x)*asp,(uv.y-u_mouse.y))));",
      "  float d=smoothstep(0.30,0.87,f);",
      "  d=d*(0.5+0.85*core)+0.04 + coreI*0.34 + mb*0.14;",
      "  d=clamp(d,0.0,1.0);",
      "  vec3 cB=vec3(0.045,0.026,0.028), cW=vec3(0.200,0.048,0.070), cR=vec3(0.340,0.078,0.110), cH=vec3(0.470,0.110,0.130), cG=vec3(0.840,0.668,0.408);",
      "  vec3 col=cB;",
      "  col=mix(col,cW,smoothstep(0.0,0.40,d));",
      "  col=mix(col,cR,smoothstep(0.38,0.80,d));",
      "  col=mix(col,cH,smoothstep(0.72,1.0,d));",
      "  col=mix(col,cG,smoothstep(0.90,1.0,d)*(0.16+0.5*coreI));",
      "  col += cG*coreI*0.32 + cG*mb*0.1;",
      "  float vig=smoothstep(1.5,0.28,length(vec2((uv.x-0.5)*asp,uv.y-0.5)));",
      "  col*=mix(0.4,1.0,vig);",
      "  col += (hash(uv*u_res + t)-0.5)*0.012;",
    ...
    const Q = Math.min(window.devicePixelRatio || 1, 1.5) * 0.85;
    let W = 0, H = 0;
    function resize() {
    ...
    let mx = 0.5, my = 0.55, cmx = 0.5, cmy = 0.55, scrollV = 0;
    window.addEventListener("pointermove", (e) => {
      mx = e.clientX / window.innerWidth;
      my = 1.0 - e.clientY / window.innerHeight;
    }, { passive: true });
    window.addEventListener("scroll", () => {
      scrollV = Math.min(1, (window.scrollY || window.pageYOffset || 0) / Math.max(1, window.innerHeight));
    }, { passive: true });
    ...
    let raf = 0, t0 = null, last = 0, inView = !("IntersectionObserver" in window);
  ...
  /* ---------- Preloader — cinematic "begin journey" console ----------
     Typed headline · living parallax atmosphere · counter 0→100 ·
  ...
  function runPreloader(gsap, done) {
    ...
    const bg = $(".ld-bg", pre);
    const fg = $(".ld-fg", pre);
    const box = $("[data-ld-box]", pre);
    const fill = $("[data-ld-fill]", pre);
    const countEl = $("[data-count]", pre);
    const enterBtn = $("[data-ld-enter]", pre);
    const calib = $("[data-ld-calib]", pre);
    const dots = $("[data-dots]", pre);
    const core = $("[data-ld-core]", pre);
    const rise = $$(".ld-rise", pre);

    /* —— type the headline + caption in, character by character —— */
    const typed = $$("[data-type]", pre).map((el) => ({ el, text: el.textContent }));
    typed.forEach((t) => { t.el.textContent = ""; });
    if (dots) dots.style.opacity = "0";
    let ti = 0, ci = 0, typeTimer = 0;
    const rand = () => 0.5 - Math.cos(ti * 2.4 + ci * 0.7) / 2; // deterministic jitter 0..1
    function typeNext() {
      if (ti >= typed.length) return;
      const cur = typed[ti];
      if (ci <= cur.text.length) {
        cur.el.classList.add("ld-type-caret");
        cur.el.textContent = cur.text.slice(0, ci);
        if (ci % 3 === 0) audio.tick(560 + (ci % 6) * 45, 0.016);
        ci++;
        typeTimer = window.setTimeout(typeNext, 15 + rand() * 22);
      } else {
        cur.el.classList.remove("ld-type-caret");
        const lineDots = cur.el.parentElement && cur.el.parentElement.querySelector("[data-dots]");
        if (lineDots) gsap.to(lineDots, { opacity: 1, duration: 0.3 });
        ti++; ci = 0;
        typeTimer = window.setTimeout(typeNext, 80);
      }
    }

    /* —— cursor parallax: atmosphere drifts opposite the HUD —— */
    let tx = 0, ty = 0, cx = 0, cy = 0, rafP = 0, parOn = true;
    const onMove = (e) => {
    ...
    function parLoop() {
      ...
      if (fg) fg.style.transform = "translate(" + (cx * 8) + "px," + (cy * 8) + "px)";
      rafP = requestAnimationFrame(parLoop);
    }
    window.addEventListener("pointermove", onMove);
    rafP = requestAnimationFrame(parLoop);

    /* —— enter: dissolve HUD, then zoom the loader out into the site —— */
    let entered = false, autoT = 0;
    const enter = () => {
      if (entered) return;
      entered = true;
      audio.whoosh(); audio.duck();
      clearTimeout(autoT); clearTimeout(typeTimer);
      parOn = false; cancelAnimationFrame(rafP);
      window.removeEventListener("pointermove", onMove);
      gsap.timeline({ onComplete: () => { pre.style.display = "none"; done(); } })
        .to([$(".ld-orbit", pre), $(".ld-headline", pre), $(".ld-glyphs", pre), $("[data-ld-audio]", pre), calib].filter(Boolean),
          { opacity: 0, duration: 0.4, ease: "power2.in" }, 0)
        .to(box, { opacity: 0, scale: 1.5, duration: 0.5, ease: "power2.in" }, 0.05)
        // the shared core blooms outward — handing the glowing heart to the hero
        .to([core].filter(Boolean), { scale: 2.8, duration: 1.0, ease: "power2.in" }, 0)
        .fromTo("#page", { opacity: 0 }, { opacity: 1, duration: 0.9, ease: "power2.out" }, 0.2)
    ...
    if (enterBtn) {
      ...
      enterBtn.addEventListener("mouseenter", () => audio.hover());
    }

    /* interactive sound toggle (synced with the persistent header one) */
    wireSoundButton($("[data-ld-audio]", pre), $("[data-ld-audio-label]", pre));
  }
  ...
  function contactSmokeGL() {
    ...
    const FRAME_MS = 1000 / 30; // cap to 30fps — the smoke drifts slowly, so this looks identical but halves GPU load
    function tick(now) {
      if (t0 === null) t0 = now;
      raf = requestAnimationFrame(tick);
      if (now - last < FRAME_MS) return;   // throttle the heavy fbm shader
      last = now;
      draw((now - t0) / 1000);
    }
    function update() {
      const run = inView && !document.hidden && !scrolling;
      if (run && !raf) { last = 0; raf = requestAnimationFrame(tick); }
      else if (!run && raf) { cancelAnimationFrame(raf); raf = 0; }
    }
    // Pause the shader while the page is actively scrolling so the GPU's frame
    // budget goes to smooth scrolling, not the (barely-moving) smoke. Resumes
    // ~150ms after scrolling settles; the time base is real-elapsed, so it never
    // jumps visibly.
    window.addEventListener("scroll", () => {
      if (!scrolling) { scrolling = true; update(); }
      clearTimeout(scrollIdle);
      scrollIdle = setTimeout(() => { scrolling = false; update(); }, 150);
    }, { passive: true });
    window.addEventListener("resize", () => { if (!raf) draw(t0 === null ? 0 : (performance.now() - t0) / 1000); }, { passive: true });
    if ("IntersectionObserver" in window && section) {
      new IntersectionObserver((ents) => { inView = ents[0].isIntersecting; update(); }, { threshold: 0 }).observe(section);
