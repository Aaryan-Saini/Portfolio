...
/* =========================================================
   GLSLHills — drifting Perlin-noise ridges (RawShaderMaterial).
   Adapted from the original full-viewport component to:
   · "use client" (WebGL at runtime)
   · fill its PARENT instead of the window (ResizeObserver)
   · cancel its RAF loop + dispose GPU resources on unmount
   · honour prefers-reduced-motion (single static frame)
   ========================================================= */
...
export interface GLSLHillsProps {
  ...
  planeSize?: number;
  speed?: number;
  /** Ridge colour as an RGB triplet in 0–1 (default neutral grey [0.6,0.6,0.6]). */
...
const GLSLHills = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      renderer.setSize(w, h, false); // match drawing buffer to container; CSS fills it
    };
  ...
  return (
    <div ref={containerRef} className={className} style={{ position: "relative", width, height, ...style }}>
      <canvas
        ref={canvasRef}
