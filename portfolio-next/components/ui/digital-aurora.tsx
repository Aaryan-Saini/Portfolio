...
export type AuroraHeroProps = {
  ...
  badgeText?: string;
  badgeLabel?: string;
  ctaButtons?: AuroraCtaButton[];
  microDetails?: string[];
};

export type AuroraRGB = [number, number, number];

/** Three-stop palette (each channel 0–1). When supplied to the shader it
 *  replaces the default rainbow with a low → mid → high gradient. */
...
export type InteractiveShaderProps = {
  flowSpeed?: number;
...
export const ShaderBackground = () => {
  const shaderProps: InteractiveShaderProps = {
    flowSpeed: 0.4,
    colorIntensity: 1.2,
    noiseLayers: 4.0,
    mouseInfluence: 0.3,
...
export const InteractiveShader = ({
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      void main() {
        ...
        for (int i=0; i<MARCH_STEPS; i++) {
            ...
            float density = map(p);
            if (density > 0.0) {
                vec3 rainbow = 0.5 + 0.5 * cos(iTime * 0.5 + p.y * 2.0 + vec3(0.0, 2.0, 4.0));
                float s = 0.5 + 0.5 * cos(iTime * 0.5 + p.y * 2.0);
                vec3 paletted = s < 0.5
                    ? mix(uColorLow, uColorMid, s * 2.0)
                    : mix(uColorMid, uColorHigh, (s - 0.5) * 2.0);
                vec3 auroraColor = mix(rainbow, paletted, uUsePalette);
    ...
    const uColorLowLocation = gl.getUniformLocation(program, "uColorLow");
    const uColorMidLocation = gl.getUniformLocation(program, "uColorMid");
    const uColorHighLocation = gl.getUniformLocation(program, "uColorHigh");
    const uUsePaletteLocation = gl.getUniformLocation(program, "uUsePalette");

    // Palette is constant for the life of the effect — set it once.
    const low = palette?.low ?? [1, 1, 1];
    const mid = palette?.mid ?? [1, 1, 1];
    const high = palette?.high ?? [1, 1, 1];
    gl.uniform1f(uUsePaletteLocation, palette ? 1 : 0);
    gl.uniform3f(uColorLowLocation, low[0], low[1], low[2]);
    gl.uniform3f(uColorMidLocation, mid[0], mid[1], mid[2]);
    gl.uniform3f(uColorHighLocation, high[0], high[1], high[2]);
    ...
    const renderLoop = () => {
      ...
      // Honour reduced-motion: draw a single static frame, don't animate.
      if (!reduce) animationFrameId = requestAnimationFrame(renderLoop);
    };
    renderLoop();
  ...
  }, [flowSpeed, colorIntensity, noiseLayers, mouseInfluence, JSON.stringify(palette)]);

  // Inline absolute-fill styles so the shader fills its parent even without
  // Tailwind (lets it be reused as a raw background layer). Callers can layer
  // on z-index / blend mode via `style`.
  return (
    <canvas
      ref={canvasRef}
      className={className}
